  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        {/* Filter Pills */}
         <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto overflow-visible">
           ...
           <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-1.5 bg-indigo-50 p-2 rounded-xl border border-indigo-200 w-full sm:w-auto min-w-0">
             <Calendar className="w-3.5 h-3.5 text-indigo-600 ml-1" />
             <select
               value={reportDateMode}
               onChange={(e) => setReportDateMode(e.target.value)}
               className="w-full sm:w-auto min-w-0 px-2 py-1.5 bg-white border border-indigo-200 rounded-lg text-xs font-bold text-slate-900 focus:outline-hidden"
             ...
             {reportDateMode === 'date' && (
               <input type="date" value={reportDate} onChange={(e) => setReportDate(e.target.value)} className="w-full sm:w-auto min-w-0 px-2 py-1.5 bg-white border border-indigo-200 rounded-lg text-xs font-bold text-slate-900" />
             )}
             {reportDateMode === 'range' && (
               <>
                 <input type="date" value={reportStartDate} onChange={(e) => setReportStartDate(e.target.value)} className="w-full sm:w-auto min-w-0 px-2 py-1.5 bg-white border border-indigo-200 rounded-lg text-xs font-bold text-slate-900" aria-label="Report start date" />
                 <span className="text-xs font-black text-indigo-700 text-center">to</span>
                 <input type="date" value={reportEndDate} onChange={(e) => setReportEndDate(e.target.value)} className="w-full sm:w-auto min-w-0 px-2 py-1.5 bg-white border border-indigo-200 rounded-lg text-xs font-bold text-slate-900" aria-label="Report end date" />
               </>
             )}
             <button type="button" onClick={() => handleDownloadLedgerPdf()} className="w-full sm:w-auto px-2.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-black cursor-pointer">
               Download
             </button>
           </div>
